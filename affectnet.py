...
class AffinityLoss(nn.Module):  # 特征亲和力损失
...
class PartitionLoss(nn.Module):  # 注意力头多样性损失
    def __init__(self, ):
...
def run_training():
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        with torch.no_grad():
            ...
            # 不同类别数的保存阈值
            if args.num_class == 7 and acc > 0.629:  # 原为0.65
            ...
            elif args.num_class == 8 and acc > 0.60:  # 原为0.62
                torch.save({'iter': epoch,
                            'model_state_dict': model.state_dict(),
                            'optimizer_state_dict': optimizer.state_dict(), },
